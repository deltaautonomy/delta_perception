...
def downsample(inputs, s_factor, stride=1, rate=1, scope=None):
    ...
    with tf.variable_scope(scope, 'Interp', [inputs]) as sc:
        ...
        return tf.image.resize_bilinear(
            inputs, [int(shrink_h), int(shrink_w)], align_corners=True
        )
...
def resnet_v1_downsample_block(scope, factor):
    """ """
    return DownSampleBlock(scope, downsample, [{'s_factor': factor}])
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                if include_root_block:
                    ...
                    net = resnet_utils.conv2d_same(
                        net, 64 // filter_scale, 7, stride=2, scope='conv1'
                    )
...
def dilated_resnet_v1_50(
  ...
  """
    ...
    blocks = [
        resnet_v1_block('block1', base_depth=64 / filter_scale, num_units=3, stride=2)
    ]

    if mid_downsample:
        blocks.append(resnet_v1_downsample_block('downsample_block', factor=0.5))

    blocks += [
        resnet_v1_block('block2', base_depth=128 / filter_scale, num_units=4, stride=2),
        resnet_v1_block(
            'block3', base_depth=256 / filter_scale, num_units=6, stride=2, rate=2
        ),
        resnet_v1_block(
            'block4', base_depth=512 / filter_scale, num_units=3, stride=1, rate=4
        ),
    ]
